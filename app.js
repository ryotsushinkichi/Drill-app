...
const UI = {
  ...
  confirmPages:   $('confirm-pages'),
  btnConfirmOk:   $('btn-confirm-ok'),
  btnConfirmBack: $('btn-confirm-back'),
...
let lastExtracted  = null; // 直近の抽出結果（確認画面用）
...
UI.btnGenerate.addEventListener('click', async () => {
  ...
  try {
    ...
    if (activeTab === 'tab-pdf') {
      ...
      extracted = await Extractor.fromPDF(file, (p, s) => setProgress(p, s));

    } else if (activeTab === 'tab-image') {
      // 画像処理
      const files = [...UI.imageInput.files];
      if (!files.length) throw new Error(I18N.t('error.no_input'));
      extracted = await Extractor.fromImages(files, (p, s) => setProgress(p, s));

    } else {
      // テキスト処理
      if (UI.textInput.value.trim().length < 20) throw new Error(I18N.t('error.text_too_short'));
      extracted = Extractor.fromText(UI.textInput.value);
    }
    ...
    lastExtracted = extracted; // 確認画面・再生成用に保持

    // 抽出結果確認画面へ
    setTimeout(() => {
      hideProcessing();
      goToConfirmScreen(extracted);
    }, 400);
...
/* ============================================================
   STEP 2: 抽出結果確認画面
   ============================================================ */
function goToConfirmScreen(extracted) {
  // 全体サマリーバッジ
  const totalPages = extracted.pages.length;
  const ocrPages   = extracted.pages.filter(p => p.usedOCR).length;
  const warnPages  = extracted.pages.filter(p => p.warning).length;

  let summaryHtml = `<span class="confirm-badge confirm-badge--ok">✓ 全${totalPages}件読み込み</span>`;
  if (ocrPages > 0) {
    summaryHtml += `<span class="confirm-badge confirm-badge--ocr">🔍 OCR使用：${ocrPages}件</span>`;
  }
  if (warnPages > 0) {
    summaryHtml += `<span class="confirm-badge confirm-badge--warn">⚠ 警告：${warnPages}件</span>`;
  }
  UI.confirmSummary.innerHTML = summaryHtml;

  // ページ別カード
  UI.confirmPages.innerHTML = extracted.pages.map(p => {
    const tag = p.usedOCR
      ? '<span class="confirm-page-tag confirm-page-tag--ocr">OCR読込</span>'
      : '<span class="confirm-page-tag confirm-page-tag--text">テキスト抽出</span>';

    const warningHtml = p.warning
      ? `<div class="confirm-page-warning">⚠ ${escHtml(p.warning)}</div>`
      : '';

    const imageHtml = p.failedImage
      ? `<img class="confirm-page-image" src="${p.failedImage}" alt="読み取り元画像" />`
      : '';

    const bodyText = p.text && p.text.length > 0
      ? escHtml(p.text)
      : '<span style="color:var(--gray-mid);">（テキストを読み取れませんでした）</span>';

    return `
      <div class="confirm-page-card ${p.warning ? 'has-warning' : ''}">
        <div class="confirm-page-head">
          <span class="confirm-page-num">${totalPages > 1 ? `${p.pageNum} / ${totalPages}` : '読み取り結果'}</span>
          ${tag}
        </div>
        <div class="confirm-page-text">${bodyText}</div>
        ${warningHtml}
        ${imageHtml}
      </div>
    `;
...
UI.btnConfirmOk.addEventListener('click', () => {
  ...
  allQuestions = Generator.generate(lastExtracted.text);

  if (allQuestions.length === 0) {
    alert(I18N.t('error.no_questions'));
    return;
  }
...
/* 確認画面：資料を選び直す */
UI.btnConfirmBack.addEventListener('click', () => {
